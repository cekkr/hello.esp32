...
static segment_handlers_t g_handlers = {0};

esp_err_t paging_init(segment_handlers_t* handlers, uint32_t max_segments) {
    ...
    memcpy(&g_handlers, handlers, sizeof(segment_handlers_t));
    g_stats.total_memory = g_handlers.get_available_memory();
    g_stats.available_memory = g_stats.total_memory;
    
    return ESP_OK;
}

esp_err_t paging_notify_segment_allocation(uint32_t segment_id, size_t size, size_t offset) {
    ...
    g_stats.available_memory = g_handlers.get_available_memory();
    
    return ESP_OK;
}

esp_err_t paging_notify_segment_access(uint32_t segment_id) {
    ...
    if (target->is_paged) {
        esp_err_t err = g_handlers.request_segment_load(segment_id);
        if (err != ESP_OK) {
            return err;
        }
    ...
    for (int i = 0; i < g_stats.num_segments; i++) {
        ...
        if (!segment->is_paged && segment->size > (32 * 1024) && 
            ...
            esp_err_t err = g_handlers.request_segment_paging(segment->segment_id, 
                                                            segment->size);
            if (err != ESP_OK) {
                return err;
            }
...
esp_err_t paging_notify_segment_modification(uint32_t segment_id) {
    for (int i = 0; i < g_stats.num_segments; i++) {
        if (g_stats.segments[i].segment_id == segment_id) {
...
esp_err_t paging_notify_segment_deallocation(uint32_t segment_id) {
    for (int i = 0; i < g_stats.num_segments; i++) {
        if (g_stats.segments[i].segment_id == segment_id) {
